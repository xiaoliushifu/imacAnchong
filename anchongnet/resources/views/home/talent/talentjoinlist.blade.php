    ...
    <div class="submain">
        ...
        <div class="server">
            ...
            <ul class="server-type">
                ...
                @foreach($sercate as $s)
                    <li><a href="{{url('server/taljoin/'.$s->id)}}">{{$s->tag}}</a></li>
                @endforeach
            ...
            <ul class="server-type">
                ...
                @foreach($adrcate as $a)
                    <li><a href="{{url('server/taljoin/'.$a->id)}}">{{$a->tag}}</a></li>
                @endforeach
            ...
            <ul class="server-type" id="adress" style="display: none">
                @foreach($addcate as $d)
                    <li><a href="{{url('server/taljoin/'.$d->id)}}">{{$d->tag}}</a></li>
                @endforeach
        ...
        <div class="subnav">
            ...
            <div class="partpage">
                ...
                <a href="
                @if(isset($joinbus))
                {{$joinbus->previousPageUrl()}}
                        @else
                {{$join->previousPageUrl()}}
                @endif ">上一页</a>&gt;
            </div>
        </div>
        <hr>
        @if(isset($joinbus))
            @foreach($joinbus as $j)
                <div class="content">
                    <div class="content-left">
                        <a href="{{url('/talent/'.$j->bid)}}"><img src="{{$j->img}}" alt=""></a>
                    </div>
                    <div class="content-right">
                        <div class="name">
                            <a href="{{url('/talent/'.$j->bid)}}"> <p>{{$j->contact}}</p></a>
                        </div>
                        <div class="content-title">
                            <h4>{{$j->title}}</h4>
                            <p><nobr>{{$j->content}}</nobr></p>
                        </div>
                    </div>
                </div>
            @endforeach
